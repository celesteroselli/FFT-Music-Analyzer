...
def level15inputs(variables, events, player):
                ...
                for i in range(num_of_elements):
                    ...
                    if (i*2) < (len(foreground)-1):
                        ...
                        if foreground[i*2][1].collidepoint(new_pos):
                            run = chord(variables["pitch_list"][i], "switch-to-m")
                            print(run)
...
def level15setup(foreground, camera):
    temp_dict = {}
...
Level_15 = Level(foreground, game_map, level15inputs, level15setup, level15dialogue, "4", "")